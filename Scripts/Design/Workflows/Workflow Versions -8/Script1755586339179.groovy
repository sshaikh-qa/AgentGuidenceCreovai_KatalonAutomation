...
WebUI.comment('Workflow Versions -8')


//String TEST_STEP= "PRECONDITION-CREATE GROUP"
//CustomKeywords.'commonKeywords.CommonKeys.createGroup'()


String TEST_STEP= "CLICK ON THE MENU ITEM AND SELECT DESIGN"
CustomKeywords.'commonKeywords.CommonKeys.navigateToMenu'('Admin')

TEST_STEP= "CLICK ON THE WORKFLOWS UNDER DESIGN"
CustomKeywords.'commonKeywords.CommonKeys.selectSubmenu'('Services')

TEST_STEP= "SELECT WORKFLOW FROM LEFT NAVIGATION"
CustomKeywords.'commonKeywords.CommonKeys.selectItemFromLeftPane'('Version Cleanup')

TEST_STEP= "VERIFY VERSION CLEANUP PAGE IS OPENED"
CustomKeywords.'servicesKeywords.ServicesKeys.verifyServicePageOpenedSuccessfully'('Version Cleanup')

TEST_STEP= "SELECT ACTIVE STATE AS ON"
CustomKeywords.'servicesKeywords.ServicesKeys.verifyServicePageOpenedSuccessfully'('Version Cleanup')
...
TEST_STEP= "CLICK ON SELECT BUTTON"
CustomKeywords.'workflowKeywords.WorkflowsKeys.clickOnSelectButtonUnderGroupTab'()

TEST_STEP= "SELECT GROUP FROM THE GROUPLIST CONTAINER"
CustomKeywords.'workflowKeywords.WorkflowsKeys.selectGroupFromGrouplistUnderWorkflow'(GlobalVariable.groupNameGlobal)

TEST_STEP= "CLICK ON ADD BUTTON TO ADD THE GROUPNAME FROM THE GROUPLIST"
CustomKeywords.'workflowKeywords.WorkflowsKeys.clickOnButtonWorkflow_Groups_GroupList_AddCancel'('Add')
...
CustomKeywords.'groupKeywords.GroupKeys.verifyWorkflowPresentInTheList'(GlobalVariable.workflowNameGlobal)
